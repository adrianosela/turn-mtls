...
func main() {
	allocationIP := flag.String("allocation-ip", "", "IP address that TURN server will create allocations on.")
	...
	if len(*allocationIP) == 0 {
		log.Fatalf("'allocation-ip' is required")
	}
	...
	log.Printf("Allocation IP: %s", *allocationIP)
		...
		ListenerConfigs: []turn.ListenerConfig{
			{
				...
				RelayAddressGenerator: &turn.RelayAddressGeneratorStatic{
					RelayAddress: net.ParseIP(*allocationIP),
					Address:      "0.0.0.0",
				},
			},
		},
